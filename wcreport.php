...
if (strlen($_SESSION['wlogin']) == 0) {
	...
	if (isset($_POST['submit'])) {
		...
		// for 4 day freshpendingwork.php
		if ($wcaction == 'Ringing' || $wcaction == 'Not reachable' || $wcaction == 'Number Busy') {
			

			$sql = 'INSERT INTO reassign_clients (cid, reassign_date,assigned_to) VALUES((:cid), (:reassign_date),NULL) ON DUPLICATE KEY UPDATE reassign_date=(:reassign_date),assigned_to=NULL;
			UPDATE clients SET wcaction=(:wcaction),nextdate=(:nextdate),wcomment=(:wcomment) WHERE id=(:cid);';

			$query = $dbh->prepare($sql);
			$query->bindParam(':cid', $cid, PDO::PARAM_INT);

			$query->bindParam(':wcaction', $wcaction, PDO::PARAM_STR);
			$query->bindParam(':reassign_date', Date('Y-m-d', strtotime('+4 days')), PDO::PARAM_STR);
		...
		} // for instant freshpendingwork.php 
		elseif ($wcaction == 'Wrong number' || $wcaction == 'Not interested' || $wcaction == 'Looking for some other product') {
			...
			$query->bindParam(':wcaction', $wcaction, PDO::PARAM_STR);
			$query->bindParam(':reassign_date', Date('Y-m-d'), PDO::PARAM_STR);
			$query->bindParam(':nextdate', $nextdate, PDO::PARAM_STR);
			$query->bindParam(':wcomment', $wcomment, PDO::PARAM_STR);
			if ($query->execute()) {
				$msg = "Report Updated Successfully";
										...
										<div class="panel-body">
											<form method="post" class="form-horizontal" enctype="multipart/form-data">
												...
												<div class="form-group">
													...
													<div class="col-sm-4">
														<input type="date" name="nextdate" class="form-control" value="<?php if( is_null($result2->nextdate)){echo htmlentities(Date('Y-m-d'));}else echo htmlentities($result2->nextdate); ?>">
													</div>
												</div>
										...
										<div class="panel-body">
											<form method="post" class="form-horizontal" enctype="multipart/form-data" name="imgform">
												...
												<div class="form-group">
													...
													<div class="col-sm-4">
														<input type="text" name="Pickup_Date" class="form-control" readonly required value="<?php $formatexpiry_date = new DateTime($result->Pickup_Date);
																																			echo htmlentities($formatexpiry_date->format('d-m-Y')); ?>">
													</div>
												...
												<div class="form-group">
													...
													<div class="col-sm-4">
														<input type="text" name="xnextdate" class="form-control" readonly required value="<?php $formatexpiry_date = new DateTime($result->nextdate);
																																			echo htmlentities($formatexpiry_date->format('d-m-Y')); ?>">
													</div>
												</div>
