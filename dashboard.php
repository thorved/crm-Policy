											...
											<div class="panel panel-default">
												<div class="panel-body bk-primary text-light">
													<div class="stat-panel text-center">
														<?php
														$email = $_SESSION['wlogin'];
														$sql = "SELECT clients.id from clients,users,reassign_clients where clients.Caller_Code=users.caller_code and users.email=(:email)and ( clients.wcaction ='Ringing' or clients.wcaction ='Number Busy' or clients.wcaction ='Not reachable') and clients.id = reassign_clients.cid and reassign_clients.reassign_date>='" . date("Y-m-d") . "'";
														$query = $dbh->prepare($sql);
														$query->bindParam(':email', $email, PDO::PARAM_STR);
														$query->execute();
														$results = $query->fetchAll(PDO::FETCH_OBJ);
														...
														?>
														<div class="stat-panel-number h1 "><?php echo htmlentities($bg); ?></div>
														<div class="stat-panel-title text-uppercase">Fresh Pending Work</div>
													</div>
												</div>
												<a href="freshpendingwork.php" class="block-anchor panel-footer">Full Detail <i class="fa fa-arrow-right"></i></a>
											</div>
										</div>
